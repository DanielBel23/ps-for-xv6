...
// source file which manages process related functionality
// code added in line 846
#include "types.h"
#include "defs.h"
#include "param.h"
#include "memlayout.h"
...
//added by me
int
findPpid(struct proc * p, int pid)
{
	int ppid = pid == 1 ? 0 : p->parent->ns_pid;
	if(p->pids[1].pid != 0)
	{
		return p->parent->pids[0].pid;
	}
	return ppid;
}

int
findExtpid(struct proc * nsp) //finds extpid and returns it, also the fact that there is no nested container is taken into consideration
{
	if(nsp->pids[1].pid != 0)
	{
		return nsp->pids[1].pid;
	}
	return nsp->pids[0].pid;
}
...
cps108()
{
	...
	for(p=ptable.proc; p<&ptable.proc[NPROC]; p++)
	{
		...
		int pid = p->ns_pid;
		int ppid = findPpid(p, pid);
		int extPid = findExtpid(p);
		if(p->state == RUNNING)
			cprintf("%s \t %d \t RUNNING \t \t %d \t %d \t %d \n", p->name, pid, extPid, ppid, cpu_time);
		else
		{
			if(p->state == 0) //state 0 is unused process slot, thus shall not be printed
				continue;
